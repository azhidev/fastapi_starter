...
async def list_tasks(project_id: str, user=Depends(current_active_user)):
    return await repo.list_for_project(project_id)


@router.post("/", response_model=TaskRead, status_code=status.HTTP_201_CREATED)
async def create_task(project_id: str, payload: TaskCreate, user=Depends(current_active_user)):
    return await repo.create(project_id, **payload.model_dump())


@router.post("/{task_id}/toggle", response_model=TaskRead)
async def toggle_done(project_id: str, task_id: str, user=Depends(current_active_user)):
    try:
